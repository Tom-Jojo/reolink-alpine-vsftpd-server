...
if [ ! -z "$TLS_CERT" ] || [ ! -z "$TLS_KEY" ]; then
  # NEW: make data channel optional (default YES for security, but set NO for Reolink compatibility)
  FORCE_LOCAL_DATA_SSL=${FORCE_LOCAL_DATA_SSL:-YES}
  TLS_OPT="-orsa_cert_file=$TLS_CERT -orsa_private_key_file=$TLS_KEY -ossl_enable=YES -oallow_anon_ssl=NO -oforce_local_data_ssl=$FORCE_LOCAL_DATA_SSL -oforce_local_logins_ssl=YES -ossl_tlsv1=NO -ossl_sslv2=NO -ossl_sslv3=NO -ossl_ciphers=HIGH"
fi
